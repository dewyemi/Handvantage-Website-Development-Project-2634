...
When writing or editing copy, watch for these specific factual claims — they're easy to get wrong because the obvious phrasing is wrong:

- **Identity is INCLUDED**, not external-only. Vantage Workspace ships with **Keycloak preconfigured**. Customers federate to Okta / Microsoft Entra ID / Auth0 / Google Workspace via OIDC if they have one; customers without an existing identity provider use Keycloak directly. **Do NOT write** "identity stays where it is" or "the platform doesn't replace identity" — both make the consolidation story weaker than it actually is. The 2026-05-09 sweep corrected this in 7 places; future drafts should not re-introduce the error.
...
The site has explicit persona-led pages alongside the technical canon. Two pages, one shared discipline. Both went through one round of revisions on 2026-05-09 after the first version got two things wrong (the identity claim and the CISO-as-buyer framing) — both corrections are now in the canon below.

- **`/for-teams`** — the business-buyer counterpart to `/architecture`. Five reading sections (CEO/Founder, CFO, COO/VP Operations, VP Sales, Department lead), each in the persona's actual vocabulary. Hero is *"Five people in the room. Five different reads of the same platform."* — frames procurement as a multi-voice negotiation, not a single-buyer purchase. Each section uses a hairline-bordered monogram (C/F/O/S/D italic in oxblood) as a chapter-marker. Includes the **consolidation table** on a midnight band, with identity correctly described as *included* via Keycloak preconfigured (federate to Okta / Microsoft Entra ID / Auth0 / Google Workspace via OIDC if one's in place). SIEM is the only piece that genuinely stays separate. No invented ROI figures — numbers are real or absent. Closes with the "compounding effect" section explaining why deployments stall when personas argue past each other.
- **`/for-ciso`** — a peer-to-peer brief written for the security leader who already understands the depth and needs help translating it for their committee. Six sections: one prepared sentence per audience (CEO/CFO/COO/board) rendered as PullQuotes with "why this lands" gloss, the risk math quantified (EU AI Act Article 99, SEC/FINRA, loss-of-certification), five committee talking points with audience-targeting, the explicit platform-vs-customer division of labour, six prepared answers to anticipated objections, and a "where to send people" routing index. Designed to be printed, forwarded, and attached to a procurement deck.

**Discoverability** (refined 2026-05-09):
- **Top nav** includes "For Teams" between "Product" and "Architecture". `/for-ciso` deliberately stays out of the top nav (the audience self-selects from `/for-teams`).
- **Home page** Section 4.5 ("What changes on Monday morning") closes with a high-visibility CTA card (paper-deep border, oxblood eyebrow, primary oxblood-filled button) pointing to `/for-teams`, with a secondary link to `/for-ciso`.
- **`/for-teams` hero** ends with a hard-to-miss CISO callout block (oxblood left-border, paper-deep background, mono eyebrow "IF YOU'RE THE CISO ON THE COMMITTEE", H4 "We wrote a 90-second brief specifically for you.", entire block is the link with hover-translate arrow). Don't bury this — security-conscious readers look for their entry point and shouldn't have to hunt.
- **Footer** has a "For your role" column listing both `/for-teams` and `/for-ciso`.

**Voice rule for these pages**: address the persona directly ("you"). Acknowledge their context (procurement dynamics, committee politics, board questions). Don't condescend — they already have judgment; the page provides language, not opinions. Don't claim the page replaces their work. **Specifically don't characterise any persona as the villain** — the previous draft framed the CISO as "the buyer" and the rest of the company as having to "use it"; both clauses were wrong and condescending. Procurement is a five-voice negotiation; every voice is legitimate.

**Persona monograms**: each persona section on `/for-teams` is marked with a hairline-bordered square containing a single italic letter in oxblood (CEO=C, CFO=F, COO=O, VP Sales=S, Department lead=D). Reads as editorial chapter-marker, not as icon. Lightweight — no images to maintain. The treatment scales: if a sixth persona is added, give it a single capital letter and the same square treatment.
